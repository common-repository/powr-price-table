<?php
    ...
    Plugin URI: https://www.powr.io/plugins/price-table
    Description: Sell more with side-by-side pricing tables  Drop the widget anywhere in your theme. Or use the POWr icon in your WP text editor to add to a page or post. Edit on your live page by clicking the settings icon. More plugins and tutorials at POWr.io.
    Author: POWr.io
    Version: 1.6.4
    ...
    */
    ...
    //////////////////////////////////////////////////////////////////////////////////////////////////
    ///////////////////////////////////////Create Price Table widget/////////////////////////////////
    class Powr_Price_Table extends WP_Widget{
      public $plugin;
      //Create the widget
      public function __construct(){
        parent::__construct( 'powr_price_table',
                             __( 'POWr Price Table' ),
                             array( 'description' => __( 'Price Table by POWr.io') )
        );
      ...
      public function widget( $args, $instance ){
        ...
        ?>
        <div class='widget powr-price-table' label='<?php echo $label; ?>'></div>
        <?php
      }
      ...
      public function form( $instance ){
        ...
        ?>
        <p>
          To edit, visit your live webpage and click the gears icon on your Price Table.
        </p>
        <p>
    ...
    function register_powr_price_table() {
      register_widget( 'Powr_Price_Table' );
    }
    //Use widgets_init action hook to execute custom function
    add_action( 'widgets_init', 'register_powr_price_table' );
    //Create short codes for adding plugins anywhere:
    function powr_price_table_shortcode( $atts ){
      if(isset($atts['id'])){
        $id = $atts['id'];
      	return "<div class='powr-price-table' id='$id'></div>";
      }else if(isset($atts['label'])){
        $label = $atts['label'];
		    return "<div class='powr-price-table' label='$label'></div>";
      }else{
      	"<div class='powr-price-table'></div>";
      }
    }
    add_shortcode( 'powr-price-table', 'powr_price_table_shortcode' );
    ...
    //ADD MENUS
    add_action( 'admin_menu', 'powr_price_table_menu' );
    function powr_price_table_menu() {
      add_menu_page( 'POWr Price Table', 'POWr Price Table', 'manage_options', 'powr-price-table-settings', 'powr_price_table_options', plugins_url('/powr-icon.png',__FILE__));
    }
    function powr_price_table_options() {
      if(powr_local_mode()){//Determine JS url:
        $redirect_url = 'https://localhost:3000/wp-create/price-table';
      }else{
        $redirect_url = 'https://www.powr.io/wp-create/price-table';
      }
    ...
    register_activation_hook(__FILE__, 'powr_price_table_plugin_activate');
      add_action('admin_init', 'powr_price_table_plugin_redirect');

      function powr_price_table_plugin_activate() {
      add_option('powr_price_table_plugin_do_activation_redirect', true);
      }
      ...
      function powr_price_table_plugin_redirect() {
      if (get_option('powr_price_table_plugin_do_activation_redirect', false)) {
          delete_option('powr_price_table_plugin_do_activation_redirect');
          if(!isset($_GET['activate-multi']))
          {
            wp_redirect( get_admin_url().'?platform=wordpress&page=powr-price-table-settings&' );
          }
       }
      }
